...
class SmiroTabBarController: UITabBarController {
    ...
    func setupUIAppTabbarController() {
        ...
        let myProfile = UINavigationController(rootViewController: MyProfileViewController())
        myProfile.tabBarItem = UITabBarItem(
            title: nil,
            image: UIImage(named: "tabbar_my")?.withRenderingMode(.alwaysOriginal),
            selectedImage: UIImage(named: "tabbar_my_s")?.withRenderingMode(.alwaysOriginal)
        )
        
        
        [chimi_home, chimi_find, chimi_mine, centerMessage, myProfile].forEach {
            $0.tabBarItem.imageInsets = UIEdgeInsets(top: 6, left: 0, bottom: -12, right: 0)
            $0.navigationBar.setBackgroundImage(UIImage(), for: .default)
            $0.navigationBar.shadowImage = UIImage()
            $0.navigationBar.isTranslucent = true
        }
        self.viewControllers = [chimi_home, chimi_find, chimi_mine, centerMessage, myProfile]
    }
